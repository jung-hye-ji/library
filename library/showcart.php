	 ...
	 <ul id="plus-menu">
		...
		<div class="dropmenu">
          ...
          <a href="question.php" class="w3-bar-item w3-button w3-xlarge">자주 묻는 질문</a>
            ...
            <a href="showcart.php" class="w3-bar-item w3-button w3-purple">대출 도서</a>
			<?php
		...
		if(!isset($_SESSION['uid'])){
			...
			$uid = $_SESSION['uid'];
			$sql = "SELECT cart.*, book.photo FROM cart, book
				WHERE cart.mybook = book.bookname AND uid = '$uid' order by no";
			$result = $conn->query($sql);
			?>
			<h2 class="bookcart">Book Cart</h2>
					<hr>
					<table id="book">
						<tr>
							<th>NO</th><th></th><th>서명</th><th>작가</th><th>출판사</th><th>대출일</th><th>반납일</th>
						</tr>
						<?php
			if(!isset($result)) die("목록 검색 오류 : ".$conn->error);
					...
					<?php } else { ?>
						<tr style="text-align: center;">
							 <td colspan='7'><?php echo "대출하신 도서가 없습니다.<br> <a href='main.php'>DJ 도서관</a>"; ?></td>
						</tr>
				<?php }	} ?>
